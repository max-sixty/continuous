...
4. When the cap hits with checks still running, end — don't re-enter the loop. The cap is the whole poll budget, not one attempt: `pending()` counts advisory jobs too, so on a repo whose nightly or benchmark matrix runs for an hour it can never return 0, and a second pass buys nothing while the runner keeps billing. Comment the still-pending checks as unverified, naming each and whether it's required, rather than exiting as if done.
