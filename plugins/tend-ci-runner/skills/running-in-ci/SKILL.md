...
Before the session starts, both harnesses restore `CLAUDE.md`, `CLAUDE.local.md`, `AGENTS.md`, `AGENTS.override.md`, `.claude/**`, and `.agents/**` at any depth from the base branch on PR events. Direct PR events carry the base ref; relayed `repository_dispatch` events carry the PR number and resolve its base ref before restoring. Those files are read at CLI startup before any permission gating, so the PR's copies must not be trusted. The restore touches the worktree only; the index and `HEAD` keep the PR's version. So on a PR that legitimately edits these paths:
