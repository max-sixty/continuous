...
- **Privileges**: Under the default `claude` harness you run as a non-sudo sandbox user, so `sudo` fails and no installer that escalates can work from inside the session. A tool that needs root belongs in the repo's `setup:` steps in `.config/tend.yaml`, which run as `runner` — with sudo — before the agent starts. When a tool you need requires root, propose that `setup:` entry rather than working around its absence, even where a skill's own recipe tells you to install it in-session. The sandbox's PATH covers system locations but never the runner's home: a tool in a system dir that's off `PATH` needs a `sandbox_path:` entry; one a `setup:` step installed under the runner's home needs `sandbox_setup:` to install or copy it into the sandbox. When a tool the repo's own gate runs is missing, propose that entry and report the gate as unrun — never substitute a weaker command (a test runner that writes snapshots rather than checking them, a lint you skip), which turns a missing tool into a silently green run.
