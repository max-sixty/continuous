...
4. When the cap hits with checks still running, end — don't re-enter the loop. The cap is the whole poll budget, not one attempt: the rollup's `pending` count includes advisory jobs too, so where a nightly or benchmark matrix runs for an hour it can never reach 0. Comment the still-pending checks as unverified rather than exiting as if done, marking each required or advisory — `gh pr checks <number> --required` lists the required contexts already registered on the commit, and an omnibus that hasn't registered yet is required too.
