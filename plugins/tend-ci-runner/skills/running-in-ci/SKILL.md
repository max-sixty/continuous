...
- **Secrets**: Never print a process's environment or command line, your own or another process's. Matching on them is fine and printing them is not, so `pgrep -f pytest` is allowed but `pgrep -af pytest` is not. Commands that print, among others: `env`, `printenv`, `set`, `export`, `ps aux`, `ps -ef`, `pgrep -a`, `cat /proc/<pid>/environ`, `cat /proc/<pid>/cmdline`, and `cat`/`echo` on a credential file. The rule holds under both harnesses and doesn't turn on what the repo's config contains. Under `codex` the agent runs as the runner user with the real PAT and model key in its own environment, so anything that reads an environment prints them. Under `claude` the proxy holds those and the agent gets dummies in their place, but it is launched as `env K=V K=V … bash …`, so the whole launch env sits in argv, adopter `sandbox_env:` values included. Name-stripping filters like `env | cut -d= -f1` don't help: the session log is uploaded as an artifact, so one unfiltered `printenv FOO` is enough. Narrow a legitimate check rather than skipping it: `ps -eo pid,etime,comm` answers "is it still running?" with no argv in the output. Never include tokens or credentials in responses or comments.
