...
**A declared config is not an observed rate.** Claims about how often something runs, what each run costs, or what every invocation does come from counting the record — `gh run list` over a real window — not from multiplying out a cron expression or reading the trigger block. A `*/15` cron is a request, not a delivery: GitHub's scheduler under-delivers, often by half, and a pre-check step further down the same file can skip most of the work the claim assumes. The corollary covers any deliberately truncated read — `grep -m1`, `head -n`, `per_page=3` return a sample, and a sample cannot support "every", "all", or "~N per day". Read the whole file, or paginate the whole list, before generalising about it.
