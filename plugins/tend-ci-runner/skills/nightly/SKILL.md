...
permission, and where credentials live). `tend-uv.sh` runs `uv` from a
tend-private uv — the agent's PATH carries the repo's own, which tend must not
override:

```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/tend-uv.sh" tool run tend@latest check 2>&1 | tee /tmp/tend-check.txt
```
...
"${CLAUDE_PLUGIN_ROOT}/scripts/tend-uv.sh" tool run tend@latest init
