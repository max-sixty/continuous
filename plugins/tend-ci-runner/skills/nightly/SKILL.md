...
permission, secrets, secret allowlist, release protection). `tend-uvx.sh` is
`uvx` backed by a tend-private uv — the agent's PATH carries the repo's own,
which tend must not override:
