...
# Stage before inspecting: `init` also writes `.github/actionlint.yaml`, which
# `git diff` cannot see while it is untracked — on an adopter that doesn't have
# it yet, the regen would otherwise read as no change at all. `.github` covers
# it and the workflow files in one pathspec that always exists.
