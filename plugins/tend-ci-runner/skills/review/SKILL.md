...
On a push or a reopen the workflow's pre-boot gate usually reaches this conclusion first and no session starts, so treat this check as the fallback it now is: it catches what the gate deliberately misses (a review carrying only inline comments), what it fails open on (an API blip), and the actions it never judges (`opened`, `ready_for_review`). The question-answering exception above can therefore no longer fire on a push — a question put to the bot reaches it through `tend-mention`, whose gate admits any comment on a PR the bot has already reviewed.
...
- In the bot's own automation (its skills, and the scheduled workflows that invoke it) and the repo's CI config: is the change worth its weight? Challenge new mechanism whose only payoff is saved compute — retries, caches, scheduling arithmetic — per **Weighing a Fix** in `running-in-ci`. A pre-boot gate is the one shape that can earn it, and only on the terms tend's `CLAUDE.md` sets: one trivial check against a frequent no-op, ahead of an agent run it saves whole. Ask for the frequency, and reject the gate that skips a rare shape or that grows a branch per case. Judge the whole change: a PR well-argued line by line can still cost more machinery than the compute it saves, and the review should say so plainly. Recommend the simple knob, or closing the PR, over refinement.
...
**Pin every review to the commit you read** — `commit_id` in every posting recipe, read back from `/tmp/reviewed-head`. Two things depend on the pin. GitHub otherwise anchors the review at whatever is live when the POST lands, so the review claims code this session never saw. And the anchor is what the queued run's pre-boot gate reads: it lists the bot's reviews on the live head, and a review pinned to the head you re-targeted onto ends that run before it allocates a runner. Unpinned, the gate finds no anchor and bills a whole session to re-review the code you just posted on. Step 1's `LAST_REVIEW_SHA` check below reads the same anchor through `bot-review-state.sh`, and catches what the gate lets through.
...
An approval you post at a re-targeted head is yours to stand behind: the queued run's gate reads that head as reviewed and never boots, so there is no successor session to dismiss the approval if a check goes red. Step 6's poll is the whole net — run it to terminal before ending the session.
...
Pushing to the branch under review fires `synchronize`, which queues another run behind this session rather than cancelling it. Submit the review (step 5) and resolve threads (step 7) before pushing, so the review documents the code the fix responds to. Poll the pushed fix's CI to green per `running-in-ci`'s "a pushed fix is always gated" before ending the session; the queued run reviews the new HEAD.
