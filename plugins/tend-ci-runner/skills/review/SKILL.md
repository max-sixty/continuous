...
This is the one exception to `running-in-ci`'s "a pushed fix is always gated" rule: you cannot poll the pushed fix's CI, because the session ends before it settles. The replacement run reads the rollup on the new HEAD in its own pre-flight.
