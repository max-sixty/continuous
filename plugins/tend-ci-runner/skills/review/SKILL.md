...
- Re-compose every `suggestion` block on a file the delta touched, and on every file after a base merge, reading the new content with `git show "$CURRENT_HEAD":<path>` — the workspace still holds the tree you reviewed, so disk gives you the old lines. A suggestion carried over unchanged reverts the author's newest edit on one click, and a multi-line one deletes every in-range line it doesn't reproduce, base-merged lines included.
