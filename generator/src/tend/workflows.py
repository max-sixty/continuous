          ...
          prompt: >-
            ...
            ${{{{ github.event_name == 'issues'
              ...
              || (github.event_name == 'pull_request_review' && contains(github.event.review.body, '@{bn}')
                && format('A review was submitted on PR #{{0}} that mentions you ({{1}}, review ID {{2}}). Read the review and full PR context (description, diff, comments, CI status), then respond. If changes were requested, make them, commit, and push. The review may include inline comments — fetch them and reply to each in its thread rather than posting a top-level PR comment.', github.event.pull_request.number, github.event.review.html_url, github.event.review.id))
              || (github.event_name == 'pull_request_review'
                && format('A review was submitted on a PR where you previously participated (PR #{{0}}, {{1}}, review ID {{2}}). Read the review and full PR context. If the review requests changes or asks questions, respond appropriately. If the review approves or is between humans, exit silently. The review may include inline comments — fetch them and reply to each in its thread rather than posting a top-level PR comment.', github.event.pull_request.number, github.event.review.html_url, github.event.review.id))
              || (contains(github.event.comment.body, '@{bn}')
